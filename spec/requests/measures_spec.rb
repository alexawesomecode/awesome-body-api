...
RSpec.describe MeasuresController, type: :controller do
  it 'creates records through post' do
    ...
    create(:measure, bodypart_id: bodypart.id, name: 'knee', value: 1)

    get :index, params: { bodypart_id: 'knee' }
    expect(JSON(response.body).size).to eq(1)
    post :create, params: { bodypart_id: 'knee', measure: {
      name: 'knee', value: 9, date: '06-07-2020', bodypart_id: bodypart.id
    } }
  ...
  it 'get right records and  associate to id succesfuly' do
    ...
    create(:measure, bodypart_id: bodypart.id, value: 22, name: 'chest')
    create(:measure, bodypart_id: bodypart.id, value: 222, name: 'chest')
    create(:measure, bodypart_id: bodypart.id, value: 233, name: 'chest')
